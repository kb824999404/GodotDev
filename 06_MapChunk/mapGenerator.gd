...
#相机对象
@onready var _camera :Node3D = %CameraPivot
#植物管理脚本
@onready var _plant_manager = %PlantManager

@onready var _reflectionProb: ReflectionProbe = %ReflectionProbe
...
func generate_map_from_file_chunks(args):
	...
	if not FileAccess.file_exists(chunks_info_path) or chunk_force_update:
		...
		# 处理植物
		if "plants" in data:
			var plant_data = data["plants"]
			for item in plant_data:
				var grid_coord = Vector2(item["pos"][1],item["pos"][0])
				var chunk_coord = grid_to_chunk_coord(grid_coord)
				var chunk_key = [ chunk_coord.y,chunk_coord.x  ]
				var chunk_info = chunks_info[chunk_key]
				var plant_name =  "{0}_{1}_{2}".format([item["name"],grid_coord.y,grid_coord.x])
				var plant_pos = Vector3(grid_coord.x, 0, grid_coord.y)* block_size + map_offset - chunk_info["center"]
				plant_pos.y = 1
				chunk_info["plants"].append({
					"type": item["type"],
		...
		for key_str: String in data:
			...
			for plant in info["plants"]:
				plant["position"] = FormatHelper.array_to_vector(FormatHelper.str_to_array_float(plant["position"]))
				chunk_plants.append(plant)
			chunks_info[chunk_key] = {
				"name": "chunk_"+"{0}_{1}".format(chunk_key),
				"center": chunk_center,
				"blocks": chunk_blocks,
...
func load_chunk(args: Array) -> void:
	...
	var chunk = Chunk.new(block_dict,info.center,info.blocks,info.plants,_camera,_plant_manager)
